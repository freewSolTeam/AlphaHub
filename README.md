...
AlphaHub is a **degen community calls** marketplace for:
- Public calls (free / open)
- Private / VIP calls (paid, smart contract checkout)
...
- **Production env (e.g. Vercel)**
  ...
  - Set `AUTH_URL` to your production domain (e.g. `https://www.alphahub.space`).
...
- **Wallet + smart contract**
  - Test wallet connect.
  - Copy contract vars from `.env.example` (section *Smart contract*) and `contracts/.env.example`.
  - Deploy: `npm run contracts:deploy` → set `ESCROW_CONTRACT_ADDRESS` in `.env`.
  - Creator payout wallets are set per operator in the dashboard (not in `.env`).
  - Test buying a private/VIP call until the on-chain order is recorded.
