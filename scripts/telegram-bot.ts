/**
 * AlphaHub Telegram worker: /start?start=link_… binding + periodic kick for expired VIP access.
 *
...
async function main() {
  ...
  bot.start(async (ctx) => {
    ...
    if (pl.startsWith("setgroup_")) {
      ...
      await ctx.reply(
        [
          ...
          "Requires: you opened this link, and the seller is linked in AlphaHub (Dashboard → Create link). Not in private chat with the bot.",
    ...
    if (!pl.startsWith("link_") || pl.length < 8) {
      await ctx.reply(
        "AlphaHub — sign in on the site (Telegram) or link this account: Dashboard → Create link. The bot can auto-kick in groups when configured.",
      );
      return;
    }
