...
function TxLink({ signature, label }: { signature: string | null; label: string }) {
  ...
  return (
    <a
      href={`https://robinhoodchain.blockscout.com/tx/${signature}`}
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex max-w-full items-center gap-1 font-mono text-[11px] text-brand/90 transition hover:text-sky-300"
      title={signature}
    >
...
function EarningsCell({ amount }: { amount: number }) {
  if (amount <= 0) {
    return <span className="text-zinc-600">—</span>;
  }
  return (
    <span className="text-zinc-300">
      <span className="inline-block tabular-nums">{formatEscrowAmountLabel(amount)}</span>
    </span>
  );
}
      ...
      >
        ...
        <div className="lb-scroll min-h-0 flex-1 overflow-auto px-4 py-3 sm:px-5 sm:py-4">
          {community.sales.length === 0 ? (
            <p className="text-xs text-zinc-500">No completed sales for this Degen yet.</p>
          ) : (
                  ...
                  {community.sales.map((sale) => (
                    <tr key={sale.id} className="border-b border-white/[0.06] last:border-0">
                      ...
                      <td className="px-3 py-2 text-right tabular-nums text-zinc-300">
                        {formatEscrowAmountLabel(sale.amount)}
                      </td>
                      <td className="px-3 py-2 text-right tabular-nums text-brand/90">
                        {formatEscrowAmountLabel(sale.netEarnings)}
                      </td>
        ...
        <table className="w-full min-w-[720px] border-collapse text-left text-xs">
          <thead>
            <tr className="border-b border-white/10 bg-zinc-900/60 text-xs font-medium uppercase tracking-widest text-zinc-500">
              <th className="px-3.5 py-2.5 font-medium">Degen</th>
              ...
              return (
                <tr key={c.id} className="border-b border-white/[0.06] last:border-0">
                  ...
                  <td className="px-3.5 py-2.5 text-right">
                    <EarningsCell amount={c.solEarnings} />
                  </td>
                  <td className="px-3.5 py-2.5 text-right">
                    <button
                      type="button"
