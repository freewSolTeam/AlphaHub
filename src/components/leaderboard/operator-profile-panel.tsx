    ...
    <div className="space-y-4 sm:space-y-5">
      <section className="rounded-lg border border-white/10 bg-zinc-950/60 p-4 sm:p-5">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
          {[
            ...
            <div key={label} className="rounded-lg border border-white/8 bg-zinc-900/40 px-3 py-2.5">
              ...
              {isEarnings ? (
                <div className="mt-1 space-y-0.5 text-sm font-semibold tabular-nums text-brand">
                  {earnings.solNet > 0 ? (
                    <p>{formatEscrowAmountLabel(earnings.solNet)}</p>
                  ) : (
        ...
        <p className="mt-3 text-xs text-zinc-600">
          Earnings are net after platform fees from completed on-chain sales.
        </p>
      </section>
      ...
      <section className="overflow-hidden rounded-lg border border-white/10 bg-zinc-950/60">
        <div className="border-b border-white/10 px-3.5 py-2.5">
          <h2 className="ui-form-section-title text-sm">Degens</h2>
        ...
        {communities.length === 0 ? (
          <p className="px-3.5 py-10 text-center text-xs text-zinc-500">No published Degens yet.</p>
        ) : (
