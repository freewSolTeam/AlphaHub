...
function StarRow({ n }: { n: number }) {
  return (
    <span className="text-brand" aria-label={`${n} of 5`}>
      {"★".repeat(n)}
      <span className="text-[var(--rh-subtle)]">{"☆".repeat(5 - n)}</span>
    </span>
  );
}

function ReviewListItem({ r, small }: { r: CommunityReviewItem; small?: boolean }) {
  const tName = small ? "text-xs font-medium text-[var(--rh-foreground)]" : "text-sm font-medium text-[var(--rh-foreground)]";
  const tTime = small ? "text-[10px] text-[var(--rh-muted)]" : "text-xs text-[var(--rh-muted)]";
  const tComment = small
    ? "mt-1 text-[11px] leading-relaxed text-[var(--rh-muted)] sm:text-xs"
    : "mt-1.5 text-sm leading-relaxed text-[var(--rh-muted)] sm:text-sm";
      ...
      <div className="flex items-start gap-2 sm:gap-2.5">
        {r.buyer.image ? (
          <CreatorAvatar
            ...
            width={avSm}
            height={avSm}
            className={`${av} shrink-0 rounded-full border border-[var(--rh-border)] object-cover`}
          />
        ) : (
          <div className={`${av} shrink-0 rounded-full border border-[var(--rh-border)] bg-[var(--rh-surface-elevated)]`} />
        )}
        <div className="min-w-0 flex-1">
          ...
          {r.imageUrl ? (
            <div
              className={`relative overflow-hidden rounded border border-[var(--rh-border)] ${
          ...
          {r.sellerReply?.trim() && r.sellerRepliedAt ? (
            <div
              className={`mt-2 border-l-2 border-brand/35 bg-[var(--rh-surface-elevated)] pr-1 pl-2.5 ${
            ...
            >
              <p
                className={
                  small
                    ? "text-[9px] font-medium uppercase tracking-wide text-brand/80/80"
                    : "text-xs font-medium uppercase tracking-wide text-brand/80/80"
                }
              >
                Operator reply
              </p>
              <div className="mt-1 flex items-start gap-1.5 sm:gap-2">
                {r.operator.image ? (
                  <CreatorAvatar
                    ...
                    width={small ? 20 : 24}
                    height={small ? 20 : 24}
                    className={`${small ? "h-5 w-5" : "h-6 w-6"} shrink-0 rounded-full border border-[var(--rh-border)] object-cover`}
                  />
                ) : (
                  <div
...
export function CommunityReviewsSection({ items, averageRating, count, embedInTabs = false }: Props) {
  if (count === 0) {
    const empty = (
      <p
        className={
          embedInTabs
            ? "text-[10px] leading-relaxed text-[var(--rh-muted)] sm:text-[11px]"
            : "px-4 py-4 text-sm leading-relaxed text-[var(--rh-muted)] sm:px-5 sm:py-4.5"
        }
      >
        No reviews yet. After checkout completes on-chain, buyers can leave a rating and note from the dashboard.
      </p>
    );
    if (embedInTabs) {
      return (
  ...
  const helpLine = (compact: boolean) => (
    ...
    >
      From buyers who completed smart contract checkout (verified). Operators can reply to reviews from the dashboard.
    </p>
  );
  ...
  const list = (small: boolean) => (
    <ul className="divide-y divide-[var(--rh-border)]" aria-label="Review list">
      {items.map((r) => (
  ...
  if (embedInTabs) {
    const body: ReactNode = (
      <>
        <div className="mb-2.5 border-b border-[var(--rh-border)] pb-2.5">
          {ratingRow(true)}
          {helpLine(true)}
        </div>
        {list(true)}
