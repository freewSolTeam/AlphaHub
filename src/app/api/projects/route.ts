...
import { sellerPayoutMissing, SELLER_PAYOUT_REQUIRED_MSG } from "@/lib/seller-payout-guard";
...
export async function POST(request: Request) {
  ...
  if (!session.user.wallet) {
    return NextResponse.json(
      { error: "Connect your Robinhood Chain wallet in the dashboard before publishing a listing." },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    if (paid) {
      const seller = await prisma.user.findUnique({
        ...
        select: { wallet: true, payoutWallet: true },
      });
      if (sellerPayoutMissing(seller)) {
        return NextResponse.json({ error: SELLER_PAYOUT_REQUIRED_MSG }, { status: 400 });
      }
    }
